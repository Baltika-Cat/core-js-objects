...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    const pairs = Object.entries(objects[i]);
    pairs.forEach((pair) => {
      if (obj[pair[0]]) {
        obj[pair[0]] += objects[i][pair[0]];
      } else {
        obj[pair[0]] = objects[i][pair[0]];
      }
    });
...
function makeWord(lettersObject) {
  ...
  const max =
    Object.values(lettersObject)
      .flat()
      .sort((a, b) => b - a)[0] + 1;
  const arr = new Array(max).fill('a');
  const pairs = Object.entries(lettersObject);
  pairs.forEach((pair) => {
    pair[1].forEach((index) => {
      arr.splice(index, 1, pair[0]);
    });
  });
  return arr.join('');
}
